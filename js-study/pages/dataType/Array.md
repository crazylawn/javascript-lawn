...
단일 느낌표(!) 는 오른쪽에 붙은 피연산자를 true 혹은 false 로 변환해주는 기능을 가지고 있다.
피연산자가 truthy 인지 falsy 인지에 따라 결과가 달라진다.

자바스크립트가 갖고 있는 논리 연산자는 다양하게 있는데, 그중에서 느낌표(!) 에 대해서 알아보자
사용빈도는 많이 높으며 단일느낌표는 오른족에 붙은 피연산자를 true 혹은 false 로 변환해주는 기능을 갖고 있다.

!value 라는 표현식에서 value 가 truthy 라면 결과는 false , value 가 falsy 라면 결과는 true 가되고, 반대의 경우도 마찬가지다
